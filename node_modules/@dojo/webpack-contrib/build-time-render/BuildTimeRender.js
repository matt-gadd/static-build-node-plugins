        ...
        compiler.hooks.afterEmit.tapAsync(this.constructor.name, (compilation, callback) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				yield page.exposeFunction('__communicator', async (modulePath, args) => {
					...
					const file = manifest[`${modulePath}.js`];
					let ex = fs_extra_1.readFileSync(path_1.join(this._output, file), 'utf-8');

					if (res && typeof res.then === 'function') {
						const asyncRes = await res;
						ex = ex.replace('/** @preserve __cache **/', `window.__cache['${modulePath}']['${JSON.stringify(args)}'] = { async: true, value: ${JSON.stringify(asyncRes)} };\n/** @preserve __cache **/`);
						fs_extra_1.writeFileSync(path_1.join(this._output, file), ex, 'utf-8');
						return asyncRes;
					}

					ex = ex.replace('/** @preserve __cache **/', `window.__cache['${modulePath}']['${JSON.stringify(args)}'] = { value: ${JSON.stringify(res)} };\n/** @preserve __cache **/`);
					fs_extra_1.writeFileSync(path_1.join(this._output, file), ex, 'utf-8');
					return res;
				});
