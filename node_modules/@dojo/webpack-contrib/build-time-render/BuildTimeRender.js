        ...
        compiler.hooks.afterEmit.tapAsync(this.constructor.name, (compilation, callback) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            try {
				...
				yield page.exposeFunction('__communicator', async (modulePath, args) => {
					const res = await require(`${process.cwd()}/src/${modulePath}`).default(...args);
					const manifest = JSON.parse(fs_extra_1.readFileSync(path_1.join(this._output, 'manifest.json'), 'utf-8'));
					const f = path_1.join('src', `${modulePath}.js`);
					const file = manifest[f];
					let ex = fs_extra_1.readFileSync(path_1.join(this._output, file), 'utf-8');
					ex = ex.replace('/** @preserve __cache **/', `window.__cache['${modulePath}']['${JSON.stringify(args)}'] = ${JSON.stringify(res)};\n/** @preserve __cache **/`);
