            ...
            try {
				...
				yield page.exposeFunction('__communicator', async (modulePath, args) => {
					...
					Object.keys(manifest).forEach((filename) => {
						if (/\.js$/.test(filename)) {
							const contents = fs_extra_1.readFileSync(path_1.join(this._output, filename), 'utf-8');
					...
					if (res && typeof res.then === 'function') {
						...
						ex = ex.replace(`/** @preserve build-cache '${modulePath}' **/`, `window.__cache['${modulePath}']['${JSON.stringify(args)}'] = { async: true, value: ${JSON.stringify(asyncRes)} };\n/** @preserve build-cache '${modulePath}' **/`);
						fs_extra_1.writeFileSync(path_1.join(this._output, file), ex, 'utf-8');
						return asyncRes;
					}

					ex = ex.replace(`/** @preserve build-cache '${modulePath}' **/`, `window.__cache['${modulePath}']['${JSON.stringify(args)}'] = { value: ${JSON.stringify(res)} };\n/** @preserve build-cache '${modulePath}' **/`);
					fs_extra_1.writeFileSync(path_1.join(this._output, file), ex, 'utf-8');
					return res;
				});
