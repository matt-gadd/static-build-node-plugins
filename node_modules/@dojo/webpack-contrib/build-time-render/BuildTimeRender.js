...
class BuildTimeRender {
    ...
    _writeIndexHtml({ html, script, path = '', styles, cache }) {
		...
		html = html.replace('<body>', `<body><script>window.__cache = ${JSON.stringify(cache)}</script>`);
    ...
    _getRenderResult(page, path = undefined, allContents = true) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
			...
			const cache = yield page.evaluate(`window.__cache`);
            return { html, styles, script, path, cache };
        });
    }
    apply(compiler) {
        if (!this._root) {
