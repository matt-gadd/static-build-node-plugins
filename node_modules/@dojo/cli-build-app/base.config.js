    ...
    const config = {
        ...
        plugins: removeEmpty([
			new webpack.NormalModuleReplacementPlugin(/\.build/, (resource) => {
				const modulePath = path.join(resource.context, resource.request).replace(basePath, '').replace(/^\//, '');
				resource.request = `@dojo/webpack-contrib/build-time-render/build-loader?modulePath=>'${modulePath}'!@dojo/webpack-contrib/build-time-render/bridge`;
			}),
            singleBundle &&
